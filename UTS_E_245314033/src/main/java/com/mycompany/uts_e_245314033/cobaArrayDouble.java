 ...
 * @author Celvin Pati
 */
public class cobaArrayDouble {//awal program
    public static void main(String[] args) {//awal metode utama
        ...
        System.out.print("Masukkan jumlah data (N): "); // Meminta pengguna untuk memasukkan jumlah data
        int N = Keyboard.nextInt(); // Membaca jumlah data dari input pengguna

        double[] hitung = new double[N]; // Deklarasi dan inisialisasi array 'hitung' dengan ukuran N
        double[] cadangan = new double[N]; // Deklarasi dan inisialisasi array 'cadangan' dengan ukuran N

        // Membaca N data bertipe double dari pengguna
        System.out.println("Masukkan " + N + " data bertipe double:"); // Menampilkan pesan untuk meminta input
        for (int i = 0; i < N; i++) { // Loop untuk N iterasi
            System.out.print("Data ke " + (i + 1) + " : "); // Menampilkan nomor data yang diminta
            hitung[i] = Keyboard.nextDouble(); // Membaca input dari pengguna dan menyimpannya ke dalam array 'hitung'
        }

        // Menyalin isi array 'hitung' ke array 'cadangan'
        for (int i = 0; i < N; i++) { // Loop untuk setiap elemen array 'hitung'
        ...
        for (int i = 0; i < N; i++) { // Loop untuk menampilkan data dari 'hitung'
            System.out.println("Data ke " + (i + 1) + " adalah " + hitung[N - 1 - i]); //Menampilkan data dari akhir ke awal
        }

        // Menampilkan isi array 'cadangan'
        System.out.println("\nData dari array 'cadangan':");
        for (int i = 0; i < N; i++) { // Loop untuk menampilkan data dari 'cadangan'
        ...
        double jumlah = 0; // Variabel untuk menyimpan jumlah elemen
        double maksimum = hitung[0]; // Inisialisasi nilai maksimum dengan elemen pertama
        double minimum = hitung[0]; // Inisialisasi nilai minimum dengan elemen pertama

        for (int i = 0; i < N; i++) { // Loop untuk menghitung jumlah, maksimum, dan minimum
        ...
        double rataRata = jumlah / N; // Menghitung rata-rata
